...
export default function PedidosPage() {
  ...
  // 🔹 Cambios en productos del formulario
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const productosValidos = productos.filter(
      (p) => p.nombre.trim() !== "" && p.precio > 0 && p.cantidad > 0
    );

    if (productosValidos.length === 0) {
      setError("Agrega al menos un producto con precio y cantidad válidos");
      return;
    }
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="bg-white border rounded-lg p-4 shadow-sm">
        ...
        {!loading && pedidosFiltrados.length === 0 && <p>No hay pedidos aún.</p>}
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {pedidosFiltrados.map((p) => {
                const subtotal =
                  p.subtotal ??
                  (p.total ? Number((p.total / 1.12).toFixed(2)) : 0);
                const iva =
                  p.iva ?? (p.total ? Number((p.total - subtotal).toFixed(2)) : 0);
                const descuentoPorcentaje = p.descuento_porcentaje ?? 0;
                const descuento = p.descuento ?? 0;
                const total = p.total ?? subtotal + iva - descuento;
                ...
                return (
                  <tr key={p.id} className="border-b hover:bg-gray-50 align-top">
                    ...
                    <td className="p-2 text-sm">
                  <select
                    aria-label={`Cambiar estado del pedido ${p.id}`}
